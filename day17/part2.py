...
def simulate(ra, rb, rc, instructions): 
    output = []
    ip = 0
...
def solve(instructions):
    # increment each digit until we reach the solution
    ra = sum(7 * (8 ** i) for i in range(len(instructions) - 1)) + 1
    while True:
        result = simulate(ra, 0, 0, instructions)
...
assert main('sample2.txt') == 117440
r = main('input.txt')
print(f'answer: {r}')